...
MATTING_REPO="${MATTING_REPO:-$(cd "${GRPO_ROOT}/.." && pwd)/matting}"

# Flow-GRPO output layout:
#   ${CHECKPOINT_DIR}/checkpoint-*/lora/adapter_model.safetensors
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${GRPO_ROOT}/outputs/flow_grpo/checkpoints}"
MODEL_ROOT="${MODEL_ROOT:-${MATTING_REPO}/FLUX.1-Kontext-dev}"
GT_ROOT="${GT_ROOT:-${MATTING_REPO}/datasets/P3M-10k}"
FILE_LIST="${FILE_LIST:-${MATTING_REPO}/data_split/P3M_matting/filenames_val_NP.txt}"
OUTPUT_DIR="${OUTPUT_DIR:-${GRPO_ROOT}/outputs/flow_grpo_eval}"
DEVICE="${DEVICE:-cuda:0}"
DATASET_NAME="${DATASET_NAME:-p3m-np}"
EVAL_LOG="${EVAL_LOG:-${OUTPUT_DIR}/evaluation_all_checkpoints.txt}"
INFERENCE_SCRIPT="${INFERENCE_SCRIPT:-${SCRIPT_DIR}/batch_inference.py}"
CONVERT_SCRIPT="${CONVERT_SCRIPT:-${SCRIPT_DIR}/convert_flux_peft_lora_to_e2p.py}"
...
for ckpt_dir in $(printf "%s\n" "${checkpoint_paths[@]}" | sort -V); do
    ...
    MATTING_REPO="${MATTING_REPO}" python "${INFERENCE_SCRIPT}" \
    ...
    PYTHONPATH="${MATTING_REPO}:${PYTHONPATH:-}" python -m utils.eval_matting \
        --pred_path "${out_root}" \
        --gt_path "${GT_ROOT}" \
