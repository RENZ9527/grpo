...
#         # Keep the reward aligned with the final matting benchmark instead of
#         # letting MSE/MAD dominate while SAD/Conn barely move.
#         weights = {
#             "mse": 0.20,
#             "mad": 0.25,
#             "sad": 0.25,
...
#             mse, mad, sad, grad, conn, pixel = _matting_metrics(pred, alpha, trimap)
#             metric_values = {
#                 "mse": mse,
#                 "mad": mad,
#                 "sad": sad,
...
#                 for name in weights
#             )
#             reward = float(np.exp(-normalized_error))
...
#                 )
#             details["matting_unknown_pixels"].append(float(pixel))
...
def matting_score(device):
    def _fn(images, prompts, metadata):
        ...
        scores = []
        details = defaultdict(list)
        # Main terms follow E2P's final eval path:
        # eval_matting.py -> compute_matting_metrics(..., whole=True).
        # Auxiliary terms keep pressure on the trimap unknown boundary and known
        # foreground/background cleanliness.
        weights = {
            "whole_mse": 0.15,
            "whole_mad": 0.15,
            "whole_sad": 0.20,
