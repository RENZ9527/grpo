    ...
    if accelerator.is_main_process:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for key, value in all_rewards.items():
                ...
                if len(valid) > 0:
                    print(f"{key}: mean={valid.mean():.6f}, std={valid.std():.6f}, min={valid.min():.6f}, max={valid.max():.6f}, n={len(valid)}")
                else:
...
def main(_):
    ...
    while True:
        ...
        if accelerator.is_local_main_process:
            print("\n🔍 [Debug] Raw Rewards: ", gathered_rewards['avg'])
